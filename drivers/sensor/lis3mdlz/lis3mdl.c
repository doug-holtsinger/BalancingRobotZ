...
#define DT_DRV_COMPAT st_lis3mdlz_magn
...
LOG_MODULE_REGISTER(LIS3MDLZ, CONFIG_SENSOR_LOG_LEVEL);
...
int lis3mdl_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	__ASSERT_NO_MSG(chan == SENSOR_CHAN_ALL || chan == SENSOR_CHAN_DIE_TEMP || chan == SENSOR_CHAN_MAGN_XYZ);
...
int lis3mdl_init(const struct device *dev)
{
	...
	/* check if CONFIG_LIS3MDLZ_ODR is valid */
	for (idx = 0U; idx < ARRAY_SIZE(lis3mdl_odr_strings); idx++) {
		if (!strcmp(lis3mdl_odr_strings[idx], CONFIG_LIS3MDLZ_ODR)) {
			break;
		}
	}
...
DT_INST_FOREACH_STATUS_OKAY(LIS3MDLZ_DEFINE)
